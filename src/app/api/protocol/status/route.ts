 ...
 * Output: { paused: [{ subsystem, reason, since }], moneyFlows: { allowed, reason? }, unavailable?: true }.
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ paused, moneyFlows }, { headers: { "Cache-Control": "public, s-maxage=5, stale-while-revalidate=10" } });
  } catch {
    return NextResponse.json({ paused: [], moneyFlows: { allowed: true }, unavailable: true }, { headers: { "Cache-Control": "no-store" } });
  }
}
