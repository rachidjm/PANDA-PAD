...
export const LEGAL_PAGES: Record<LegalSlug, LegalPage> = {
  ...
  "terms-of-service": {
    ...
    body: {
      en: [
        ...
        "Non-custodial trading and creation. PANDA never takes custody of your funds, tokens, or private keys for trading, coin creation or fee-distribution setup — each of those is a transaction you review and sign yourself, in your own wallet.",
  ...
  "privacy-policy": {
    ...
    body: {
      en: [
        ...
        "Wallet data. When you connect a wallet, PANDA reads your public wallet address and, for the Portfolio page, your real on-chain token balances — all of this is already public on the Solana blockchain and is read live each time you view those pages. PANDA also keeps a few records tied to public wallet addresses, none of them secret: the log of trades you make through PANDA (used for Portfolio) and audit entries (wallet address, action, public transaction signature). These are stored as files that anyone holding their URL can read. PANDA never receives, requests, or stores your seed phrase or private key.",
      ...
      es: [
        ...
        "Datos de la wallet. Al conectar una wallet, PANDA lee tu dirección pública y, para la página de Portfolio, tus saldos reales de tokens on-chain — todo esto ya es público en la blockchain de Solana y se lee en vivo cada vez que visitas esas páginas. PANDA también guarda algunos registros asociados a direcciones públicas de wallet, ninguno secreto: el registro de las operaciones que haces a través de PANDA (para el Portfolio) y entradas de auditoría (dirección de wallet, acción, firma pública de la transacción). Se almacenan como archivos que puede leer cualquiera que tenga su URL. PANDA nunca recibe, solicita ni almacena tu frase semilla o clave privada.",
  ...
  disclaimer: {
    ...
    body: {
      en: [
        DRAFT_NOTICE.en,
        "PANDA is a non-custodial software interface for creating and trading coins on Solana. PANDA is not a financial advisor, broker, dealer, exchange, or bank, and nothing in the app constitutes financial, investment, legal, or tax advice.",
      ...
      es: [
        DRAFT_NOTICE.es,
        "PANDA es una interfaz de software no custodial para crear y operar monedas en Solana. PANDA no es un asesor financiero, bróker, dealer, exchange ni banco, y nada en la aplicación constituye asesoramiento financiero, de inversión, legal o fiscal.",
 ...
 * wallet whose key is held by PANDA's servers, so this is the one place PANDA holds funds. With the flag off there is no
 * such flow and these must not appear (see getLegalPage).
 */
export const HOLDER_ADDENDA: Partial<Record<LegalSlug, Record<Lang, string[]>>> = {
  "legal-notice": {
    ...
    es: ["Las recompensas de holders son el único punto donde PANDA retiene fondos. Si el creador de una moneda asigna parte de sus comisiones a \"Holders\", esas comisiones se pagan on-chain a la wallet del Rewards Pool de PANDA, cuya clave custodian los servidores de PANDA; PANDA paga después a cada holder su parte cuando la reclama (reclamar requiere una firma gratuita de un mensaje desde tu wallet, no una transacción). Hasta que se reclaman, esos fondos están en esa wallet — confías su custodia a la infraestructura de PANDA."],
  },
  "terms-of-service": {
    en: ["Holder rewards are custodied by PANDA. Fees a creator assigns to \"Holders\" are paid to PANDA's Rewards Pool wallet, controlled by PANDA's servers, and paid out when holders claim them (a free message signature, not a transaction). Until then those funds are in that wallet."],
    es: ["Las recompensas de holders las custodia PANDA. Las comisiones que un creador asigna a \"Holders\" se pagan a la wallet del Rewards Pool de PANDA, controlada por sus servidores, y se pagan cuando los holders las reclaman (una firma gratuita de un mensaje, no una transacción). Hasta entonces esos fondos están en esa wallet."],
  },
  "privacy-policy": {
    en: ["Holder rewards. PANDA keeps a ledger of the rewards owed to and claimed by each wallet address, stored as files that anyone holding their URL can read."],
    es: ["Recompensas de holders. PANDA guarda un libro de las recompensas pendientes y reclamadas por cada dirección de wallet, almacenado como archivos que puede leer cualquiera que tenga su URL."],
  },
  "risk-disclosure": {
...
/** The legal page as it applies to this deployment: the base text, plus the custody sections only for the features that are on. */
export function getLegalPage(slug: LegalSlug, opts: { custody: boolean; holders?: boolean }): LegalPage {
  const base = LEGAL_PAGES[slug];
  const extras = [opts.custody ? CUSTODY_ADDENDA[slug] : undefined, opts.holders ? HOLDER_ADDENDA[slug] : undefined].filter((e): e is Record<Lang, string[]> => !!e);
  if (extras.length === 0) return base;
  return { ...base, body: { en: [...base.body.en, ...extras.flatMap((e) => e.en)], es: [...base.body.es, ...extras.flatMap((e) => e.es)] } };
}
