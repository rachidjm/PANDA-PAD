...
/** Marker for a fact only the operator can supply. Nothing is invented: it stays visible until it is filled in (see legalPlaceholders). */
const TBC = (what: string) => `[COMPLETAR: ${what}]`;
...
export const LEGAL_PAGES: Record<LegalSlug, LegalPage> = {
  "legal-notice": {
    ...
    body: {
      en: [
        DRAFT_NOTICE.en,
        `Owner of this website (Art. 10, Spanish Law 34/2002, LSSI-CE). Name or company name: ${TBC("full name or company name")}. Tax ID (NIF/CIF): ${TBC("NIF/CIF")}. Address: ${TBC("street, postcode, city, country")}. Contact email: ${TBC("email address")}. Commercial-register data, if applicable: ${TBC("register, volume, folio, sheet — or remove if not applicable")}.`,
        ...
        "Market and pool data shown in the app comes from Pump.fun's public API, Dexscreener and GeckoTerminal — independent third parties that index public Solana activity — and swap quotes come from Jupiter. PANDA sets aside coins whose data looks unreliable (for example a huge market cap on almost no liquidity); that is a data-quality filter, not a judgment on any coin.",
      ],
      es: [
        DRAFT_NOTICE.es,
        `Titular de este sitio web (art. 10 de la Ley 34/2002, LSSI-CE). Nombre o denominación social: ${TBC("nombre completo o razón social")}. NIF/CIF: ${TBC("NIF/CIF")}. Domicilio: ${TBC("calle, código postal, ciudad, país")}. Correo electrónico de contacto: ${TBC("correo electrónico")}. Datos registrales, si procede: ${TBC("registro, tomo, folio, hoja — o eliminar si no procede")}.`,
  ...
  "terms-of-service": {
    ...
    body: {
      en: [
        ...
        `Who may use PANDA. You must be at least ${TBC("minimum age")} years old, and it is your responsibility to check that using PANDA and trading crypto-assets is lawful where you live. Jurisdictions where PANDA is not offered: ${TBC("list of excluded jurisdictions, or write \"none\"")}.`,
        ...
        `Changes. These terms may be updated as PANDA evolves; the date of the last change is shown on this page, and continued use after a change means you accept the new terms. Governing law and courts: ${TBC("governing law and jurisdiction")}.`,
      ],
      es: [
        DRAFT_NOTICE.es,
        "Al usar PANDA, aceptas estos términos. Si no estás de acuerdo, no uses la aplicación.",
        `Quién puede usar PANDA. Debes tener al menos ${TBC("edad mínima")} años, y es tu responsabilidad comprobar que usar PANDA y operar con criptoactivos es legal donde vives. Jurisdicciones en las que PANDA no se ofrece: ${TBC("lista de jurisdicciones excluidas, o escribir \"ninguna\"")}.`,
        ...
        `Cambios. Estos términos pueden actualizarse a medida que PANDA evolucione; la fecha del último cambio figura en esta página, y seguir usando la app tras un cambio implica aceptar los nuevos términos. Legislación aplicable y tribunales: ${TBC("legislación aplicable y jurisdicción")}.`,
      ],
    },
  },
  "privacy-policy": {
    ...
    body: {
      en: [
        ...
        `Who is responsible. ${TBC("name of the data controller and contact email — the same as in the Legal Notice")}.`,
      ...
      es: [
        ...
        `Responsable. ${TBC("nombre del responsable del tratamiento y correo de contacto — el mismo del Aviso Legal")}.`,
        ...
        "Dónde se guarda. Los registros están en una base de datos Postgres alojada en Neon. Los contadores de límite de peticiones y los informes agregados de seguridad del navegador están en Upstash (Redis). Las imágenes y metadatos que subes para una moneda — y los archivos de cualquier función que aún no esté en la base de datos — están en Vercel Blob, en direcciones públicas. El propio sitio se ejecuta en Vercel. Las funciones y la base de datos funcionan en Estados Unidos (US East), y Upstash y Vercel son empresas estadounidenses, así que tus datos se transfieren fuera de la Unión Europea. Durante el traslado del almacenamiento de archivos a la base de datos, quedan copias antiguas de algunos registros en Vercel Blob durante un periodo de transición, tras el cual se eliminan.",
        `Cuánto tiempo. Las sesiones duran 2 horas; las sesiones y los desafíos de inicio de sesión caducados se borran automáticamente (los desafíos un día después de caducar, las sesiones una semana después). Los contadores de límite de peticiones caducan con su ventana. Las entradas de auditoría no se pueden editar ni borrar — la base de datos lo rechaza, por diseño, para que el historial no pueda reescribirse. Los registros de operaciones, actividad y lanzamientos se conservan durante ${TBC("plazo de conservación")}.`,
...
export const CUSTODY_ADDENDA: Addenda = {
  ...
  "terms-of-service": {
    en: [
      "Custodial exception: Stop Loss / Take Profit and Draw Your Trade. Setting one up deposits your tokens (or the SOL / USDC that will fund the buy) into a Jupiter Trigger vault — a Privy-managed custodial account, not PANDA's — until the order fills or you cancel it. Cancelling requires your signature and returns the funds to your wallet. Using this feature is optional and you accept the custody terms of Jupiter and Privy when you do.",
...
/** Every "[COMPLETAR: …]" still in the texts (with all optional sections on): what the operator still has to supply. */
export function legalPlaceholders(): { slug: LegalSlug; what: string }[] {
  const all: LegalOptions = { custody: true, holders: true, otc: true, nft: true, market: true, points: true, airdrops: true, pandaToken: true };
  return LEGAL_SLUGS.flatMap((slug) =>
    getLegalPage(slug, all).body.es.flatMap((p) => [...p.matchAll(/\[COMPLETAR: ([^\]]+)\]/g)].map((m) => ({ slug, what: m[1] })))
  );
}
