...
export const ENV_SPECS: EnvSpec[] = [
  ...
  { names: ["UPSTASH_REDIS_REST_URL", "KV_REST_API_URL"], group: "storage", required: (e) => e.NODE_ENV === "production", valid: isHttpUrl, affects: "rate limiting (Upstash Redis): in production, without it every money route (trades, launches, claims, NFT mint/market, strategies, sending transactions) is REFUSED with 503; other routes fall back to a per-instance limiter" },
  { names: ["UPSTASH_REDIS_REST_TOKEN", "KV_REST_API_TOKEN"], group: "storage", required: (e) => e.NODE_ENV === "production", valid: nonEmpty, affects: "rate limiting (Upstash Redis): see UPSTASH_REDIS_REST_URL" },
  { names: ["CSP_MODE"], group: "core", required: never, valid: (v) => ["report-only", "enforce", "off"].includes(v.trim().toLowerCase()), affects: "the Content-Security-Policy on pages: report-only (default: browsers report what they would block, nothing is blocked), enforce (blocks) or off. Only enforce once the reports in /admin are clean" },
