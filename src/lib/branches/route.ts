 ...
 */
export function branchGate(req: Request, key: string, perWallet = 20): { wallet: string } | NextResponse {
  ...
  if (rateLimited(`branch:${key}:wallet:${wallet}`, perWallet, 60_000) || rateLimited(`branch:${key}:ip:${clientIp(req)}`, 60, 60_000)) {
